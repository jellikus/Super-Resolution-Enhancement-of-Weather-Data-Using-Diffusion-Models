...
class PhySRDiffDiffusion(GaussianDiffusion):
    """
    A class for PhySRDiff diffusion model.
    Args:
    ...
    def p_sample_loop(self, x_in, continous=False):
        """
        Implementation of SRDiff diffusion sampling algorithm.
        Args:
        ...
        if not self.conditional:
            ...
            for i in reversed(range(0, self.num_timesteps)):
                img = self.p_sample(img, i, condition_x=(x_interpolated, rrdb_encoded))
                if i % sample_inter == 0:
                    # ret_img = torch.cat([ret_img, img], dim=0)
                    ret_img = img
        if continous:
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        """
        Conditions srdiff in right way with conv filters and compute:
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn((img_lr_up, rrdb_encoded, x_noisy), continuous_sqrt_alpha_cumprod)
