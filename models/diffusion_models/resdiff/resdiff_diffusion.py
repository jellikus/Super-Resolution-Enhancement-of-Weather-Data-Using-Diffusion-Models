...
class ResDiffDiffusion(GaussianDiffusion):
    """
    A class for ResDiff diffusion model that inherits from general GaussianDiffusion class.
    Args:
    ...
    def __init__(
        ...
        if pretrained_model_path is not None:
            self.cnn = SimpleCNN(scale_factor=4, channels=channels)
            self.cnn.load_state_dict(torch.load(pretrained_model_path))
            self.cnn.eval()
            if lock_weights:
    ...
    def p_losses(self, x_in, noise=None):
        """
        Calculate the training loss for a single step of the reverse diffusion process for ResDiff model
        including denoise fucntion utilization and simple CNN.
