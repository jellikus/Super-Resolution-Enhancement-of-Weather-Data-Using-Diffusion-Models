...
from ..nn_modules.functional_layers import default, exists, PositionalEncoding, Swish, Upsample, Downsample
from ..nn_modules.resnet import Block, ResnetBlocWithAttn
from .guided_cross_attention import HF_guided_CA
from .fd_info_spliter import FD_Info_Spliter


class UNet(nn.Module):
    """
    U-Net architecture for ResDiff model.
    ...
    def __init__(
        ...
        self.fd_spliter = FD_Info_Spliter(dim=inner_channel, in_channels=self.image_channels, out_channels=out_channel,
    ...
    def forward(self, x, time):
        ...
        dwt_x, _ = torch.split(x, self.image_channels, dim=1)
        ...
        # print("fd info spliter input", x.shape)
        # FD Info Spliter
        x = self.fd_spliter(x, t)
        # print("fd info spliter output", x.shape)

        feats = []
        idx = 0
        for layer in self.downs:
            if isinstance(layer, ResnetBlocWithAttn):
                # print("layer ResNetBlock")
                # print("in x:", x.shape)
                x = layer(x, t)
                # print("out x:", x.shape)
            else:
                # print("layer Downsample")
                # print("in x:", x.shape)
                x = layer(x)
                # print("out x:", x.shape)
